...
public class University {
  ...
  public University() {
//    db = DBMaker.fileDB("univ11.db").checksumHeaderBypass().fileMmapEnable().make();
    db = DBMaker.fileDB("univ11.db").fileMmapEnable().make();
  ...
  void register(String sID, String cID) {
    if (!(courseExist(cID) && studentExist(sID)))
      throw new RuntimeException("Unknown course or student");
    Registration reg = new Registration(sID, cID);
    reg.rID = "r" + getIncr();
  ...
  void listStudents() {
    ...
    for (Entry<String, Student> e : entries) {
      System.out.printf("%s Student %s (%s)\n", e.getValue().sID, e.getValue().name, e.getValue().email);
  ...
  public String[] getStudentColumns() {
    return new String[] { "sID", "Name", "Email", "Major" };
  }

  public Object getStudentData(int rowIndex, int columnIndex) {
    String[] keys = students.keySet().toArray(new String[0]);
    Student s = students.get(keys[rowIndex]);
    if (columnIndex == 0)
      return s.sID;
